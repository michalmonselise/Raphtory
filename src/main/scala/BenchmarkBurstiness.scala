...
object BenchmarkBurstiness{
  def apply(seperation:Int): BenchmarkBurstiness = new BenchmarkBurstiness(Array(seperation.toString))
}

class BenchmarkBurstiness(args: Array[String]) extends Analyser(args){
